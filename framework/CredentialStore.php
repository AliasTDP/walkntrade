...
class CredentialStore extends Walkntrade{
	private $loggedIn;
	...
	private function authorizeFromCookie(){
		if(isset($_COOKIE["sessionUid"]) && isset($_COOKIE["sessionSeed"])){
			...
			$user_browser = $_SERVER['HTTP_USER_AGENT']; // Get the user-agent string of the user.
			if ($stmt = $this->getUserConnection()->prepare("SELECT password, name, email, infractions, emailPref, phone FROM users WHERE id = ? LIMIT 1")) { 
				$stmt->bind_param('i', $user_id); // Bind "$user_id" to parameter.
				$stmt->execute(); // Execute the prepared query.
				$stmt->store_result();
				if($stmt->num_rows == 1) { // If the user exists
					$stmt->bind_result($password,$username,$emailAddress, $infractions, $emailPref, $phoneNum); // get variables from result.
					$stmt->fetch();
					$login_check = md5($password.$user_browser);
